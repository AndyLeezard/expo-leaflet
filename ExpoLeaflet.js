...
export const ExpoLeaflet = ({ wrapperStyle, webviewContainerStyle, loadingIndicator, onMessage, onMapLoad, ...rest }) => {
    var _a;
    const mapProps = rest;
    ...
    const _webviewContainerStyle = useMemo(() => {
        let styles = [
            {
                height: "100%",
                width: "100%",
        ...
        if (webviewContainerStyle) {
            styles.push(webviewContainerStyle);
        }
        return styles;
    }, [webviewContainerStyle]);
    return (<View style={_wrapperStyle}>
      {webViewContent != null && (<WebView allowFileAccess={true} allowUniversalAccessFromFileURLs={true} allowFileAccessFromFileURLs={true} containerStyle={_webviewContainerStyle} domStorageEnabled={true} javaScriptEnabled={true} ref={webViewRef} onLoadEnd={() => {
